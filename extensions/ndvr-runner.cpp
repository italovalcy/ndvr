...
NdvrRunner::NdvrRunner(std::string& networkName, std::string& routerName, int helloInterval, std::string& validationConfig, std::vector<std::string>& namePrefixes, std::vector<std::string>& faces)
{
  m_signingInfo = ndn::security::SigningInfo(ndn::security::SigningInfo::SIGNER_TYPE_ID,
                                             networkName + routerName);
  m_ndvr = std::make_shared<Ndvr>(m_signingInfo, networkName, routerName, namePrefixes, faces, validationConfig);
  if (helloInterval != 0)
...
NdvrRunner::printUsage(const std::string& programName)
{
  ...
  std::cout << "       -f <FACE>   Specify the face in which NDVR will work (can be used multiple times)" << std::endl;
