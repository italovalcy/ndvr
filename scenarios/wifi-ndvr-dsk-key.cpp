...
main(int argc, char* argv[])
{
  ...
  ::ndn::ndvr::setupRootCert(ndn::Name(network), "config/trust.cert", keyType);
  ...
  for (NodeContainer::Iterator i = nodes.Begin(); i != nodes.End(); ++i, idx++) {
    ...
    app->AddSigningInfo(::ndn::ndvr::setupSigningInfo(ndn::Name(network + routerName), ndn::Name(network), keyType));
