...
for line in file:
    if line.find("ndn.SimplePubSub:AddNewNamePrefix()") != -1:
        elements = line.split(' ')
        time = float(elements[0][1:-1])
        data_name = elements[-1]
        data_store[data_name] = DataInfo(time)
    elif line.find("ndn.SimplePubSub:OnSyncDataContent()") != -1:
        elements = line.split(' ')
        time = float(elements[0][1:-1])
        data_name = elements[-1]
        ...
        # sanity check: the number of data sync cannot be greater than num_nodes
        assert data_info.count <= args.num_nodes
        ...
        if data_info.count == args.num_nodes:
...
if args.histogram:
    hist, bin_edges = np.histogram(sync_duration,bins=[0,1,2,3,4,5,10,15,20,40,60,80,100,120,140,160,180,200])
